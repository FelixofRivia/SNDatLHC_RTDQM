...
def plotBoardHitRate(canvasName,boardNumber):
    ...
    #set hists
    hBoardRate = TH1D(f"{canvasName}Rate",f"{canvasName} Hits per {binwidth} sec",int(h.timeRange/binwidth),0,h.timeRange)
    ...
    hBoardRate.GetYaxis().SetTitle(f"hits per {binwidth} sec")
