...
def plot2DSciFi(xBoardNumber,yBoardNumber,canvasName):

    eventStart = h.eventStart
    iupdate = h.updateIndex

    #make 2Dhist assigned with channels
    hitMap = TH2D(str(canvasName)+"_2Dmap",str(canvasName)+"_2Dmap",128,0,12.8,128,0,12.8)
    
    #make canvas with n divisions, cd to the index of the canvas
    canvas = TCanvas(str(canvasName)+"_2Dmap",str(canvasName)+"_2Dmap",500,500)

    hitMap.GetXaxis().SetTitle("x (cm)")
    ...
    while(run):
        ...
        if(i >= h.eventEnd):
            ...
            canvas.Draw()
            # add evt number
            hitMap.SetTitle(f"{canvasName}_2Dmap: evt {i}")
            canvas.Modified()
            canvas.Update()
            # save on root file
            task.wrthisto(hitMap, f"{canvasName}_2Dmap")

            if i == 999999:
                print(f"{canvasName}_2Dmap event number : 999999. End of file",flush=True)
                while(h.waitingEnd):
                    t.sleep(1)
                i = h.iArr[iNext]
            #if end of a set range
        ...
        #update histograms
        if i%iupdate == 0:
            print(f"{canvasName}_2Dmap event number : {i}",flush=True)
            hitMap.Draw("colz")
            canvas.Draw()
            # add evt number
            hitMap.SetTitle(f"{canvasName}_2Dmap: evt {i}")
        ...
        if (xBoardNumber in boardArr) and (yBoardNumber in boardArr):
            for b in range(0,len(boardArr)):
                if xBoardNumber==boardArr[b]:
                    try:
                        xch.append((64*tofID[b] + 63 - tofChannel[b])*0.025)
                    except:
                        print("Ch not found in 2Dmaps...ignoring",flush=True)
                if yBoardNumber==boardArr[b]:
                    try:
                        ych.append((64*tofID[b] + 63 - tofChannel[b])*0.025)
                    except:
                        print("Ch not found in 2Dmaps...ignoring",flush=True)
        # fill every combination
        if len(xch)>0 and len(ych)>0:
